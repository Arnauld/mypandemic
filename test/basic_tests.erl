...
divide_should_return_the_division_of_two_numbers__test() ->
  Dividend = 5,
  Divisor = 2,
  Quotient = 2.5,
...
%% 3
divide_should_return_undefined_when_divisor_is_0__test() ->
  Dividend = 5,
  Divisor = 0,
  Quotient = undefined,
  ?assertEqual(Quotient, basic:divide(Dividend, Divisor)).
...
%% 4
sum_should_return_the_sum_of_numbers__test() ->
  A = 3,
  B = 4,
  ?assertEqual(7, basic:sum(A, B)).

%% 5
sum_should_return_the_sum_of_two_3D_vectors__test() ->
  A = {5, 3, 6},
  B = {1, 2, 3},
  ?assertEqual({6, 5, 9}, basic:sum(A, B)).

%% 6
sum_should_return_the_sum_all_elements_of_a_list__test() ->
  L = [1, 2, 3, 4, 5],
  ?assertEqual(15, basic:sum(L)).

%% 7
%% apply with '+' or '/' for case of
apply_should_return_the_sum_of_2_elements_when_operator_is_plus__test() ->
  Operator = '+',
  ?assertEqual(8, basic:apply(4,Operator, 4)).

%% 8
apply_should_return_the_division_of_2_elements_when_operator_is_slash__test() ->
  Operator = '/',
